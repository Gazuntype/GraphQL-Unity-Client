...
namespace GraphQlClient.Core
{
    public class HttpHandler
    {
        ...
        )
        {
            ...
            return await SendRequestAsync(request, details);
        }
        ...
        private static async Task<UnityWebRequest> SendRequestAsync(
        ...
        )
        {
            ...
            try
            {
                ...
                {
                    FireEvent(new OnRequestEnded(new Exception(request.error)));
                }
                else
                {
                    FireEvent(new OnRequestEnded(request.downloadHandler.text));
                }
            }
            catch (Exception e)
            {
                FireEvent(new OnRequestEnded(e));
            }

            return request;
        }

        private static void FireEvent<T>(T eventData)
            where T : class
        {
            var fireEventMethod = typeof(T).GetMethod("FireEvent");
            fireEventMethod?.Invoke(eventData, null);
        }
        ...
        public static async Task<ClientWebSocket> WebsocketConnect(
        ...
        static async void GetWsReturn(ClientWebSocket cws)
        {
            ...
            do
            {
                ...
                result += Encoding.UTF8.GetString(
                    buf.Array ?? throw new ApplicationException("Buf = null"),
                    buf.Offset,
                    r.Count
                );
            ...
            switch (subType)
            {
                ...
                case "data":
                {
                    OnSubscriptionDataReceived subscriptionDataReceived =
                        new OnSubscriptionDataReceived(result);
